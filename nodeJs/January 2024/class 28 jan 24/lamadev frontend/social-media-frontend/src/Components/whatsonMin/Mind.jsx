...
export default function Mind({ userProfilePic }) {
    ...
    console.log(userTextInput, "user text input")
    const PostKrRahaHo = async () => {
        try {
            ...
            console.log(post_Posting, "post posting")
            if (post_Posting.data.status) {
                console.log(post_Posting.data.message)
            } else {
                console.log(post_Posting.data.message)
            }
        } catch (error) {
        ...
        <Paper elevation={8} >
            <div className='w-196 flex flex-col items-start gap-10 shadow-lg p-4 rounded text-xs'>
                <div className='flex items-center justify-center gap-8'>
                    ...
                    <div><BasicTextFields setUserTextInput={setUserTextInput} /></div>
                </div>
                <div className='flex gap-3'>
                    ...
                    <div className='text-xs' ><Button variant="contained" onClick={PostKrRahaHo}>Share</Button></div>
                </div>
            </div>
        </Paper>
    )

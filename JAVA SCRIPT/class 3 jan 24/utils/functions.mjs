import { auth, createUserWithEmailAndPassword, db, doc, getDoc, setDoc, signInWithEmailAndPassword, ref, storage, getDownloadURL, uploadBytes, signOut, getDocs, query, collection, serverTimestamp, addDoc, orderBy, updateDoc, deleteDoc } from "./firebaseConfig.js"
...
const deletData = async (collectionName, id) => {
    try {
        //logging in user and calling firebase auth login function
        const deletting = await deleteDoc(doc(db, collectionName, id));
        return {
            status: true,
...
const addInDB = async (data, collectionName) => {
    try {
        ...
        console.log(addData, "===>>>>> add data form sir's functions")
        // dataWithTimestamp.postID = addData.id
        return {
            // postId:addData.id,
            status: true,
